...
/** So Settings can notify the main window when Unichan config is saved (re-apply without refresh). */
...
export async function setupMainWindow(params: {
  ...
  const mainWindowConfig = getConfig().windows?.find(w => w.title === 'UNICHAN' && w.tag === 'main')
  ...
  const window = new BrowserWindow({
    ...
    webPreferences: {
      ...
      sandbox: false,
      // Allow cross-origin requests to remote brain (e.g. http://192.168.0.169:18789) so chat works without CORS
      webSecurity: false,
    },
  ...
  window.on('closed', () => { mainWindowRef = null })

  if (params.onWindowCreated) {
    params.onWindowCreated(window)
  }

  // Open DevTools only when explicitly requested (avoids Chromium Autofill/Storage noise in console)
  if (env.MAIN_APP_OPEN_DEVTOOLS === '1' || env.MAIN_APP_OPEN_DEVTOOLS === 'true') {
    try {
  ...
  function handleNewBounds(newBounds: Rectangle) {
    ...
    const existingConfigIndex = config.windows.findIndex(w => w.title === 'UNICHAN' && w.tag === 'main')
    ...
    if (existingConfigIndex === -1) {
      ...
      const mainWindowConfig = defu(config.windows[existingConfigIndex], { title: 'UNICHAN', tag: 'main' })
