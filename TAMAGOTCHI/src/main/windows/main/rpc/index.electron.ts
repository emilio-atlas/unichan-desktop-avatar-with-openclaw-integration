...
import { electronOpenChat, electronOpenMainDevtools, electronOpenSettings, noticeWindowEventa, unichanConfigGet, type UnichanConfigPayload } from '../../../../shared/eventa'
...
export function setupMainWindowElectronInvokes(params: {
  ...
  const unichanConfigPath = join(homedir(), '.unichan', 'config.json')
  defineInvokeHandler(context, unichanConfigGet, async (): Promise<UnichanConfigPayload> => {
    try {
      const raw = await readFile(unichanConfigPath, 'utf-8')
      return JSON.parse(raw) as UnichanConfigPayload
    }
    catch {
      return {}
    }
