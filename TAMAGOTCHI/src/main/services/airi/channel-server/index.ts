...
import { electronGetWebSocketServerUrl, electronRestartWebSocketServer, electronStartWebSocketServer } from '../../../../shared/eventa'
import { onAppBeforeQuit } from '../../../libs/bootkit/lifecycle'

let serverInstance: { close: (closeActiveConnections?: boolean) => Promise<void> } | null = null
/** Resolved WebSocket URL (e.g. ws://localhost:6121/ws) when server is running; used by renderer when port fallback was used. */
let resolvedWebSocketUrl: string | null = null
...
const PORTS_TO_TRY = [6121, 6122, 6123, 6124, 6125]
...
export async function setupServerChannel(options?: { websocketSecureEnabled?: boolean }) {
  ...
  // If we already have a server (e.g. renderer hot-reload called start again), close it first
  // so we can bind to the same port instead of hogging 6122, 6123, …
  if (serverInstance && typeof serverInstance.close === 'function') {
    try {
      log.log('closing existing WebSocket server before (re)start')
      await serverInstance.close(true)
    }
    catch (e) {
      log.warn('error closing existing server:', (e as Error)?.message)
    }
    serverInstance = null
  }
  ...
  try {
    ...
    const basePort = env.PORT ? Number(env.PORT) : PORTS_TO_TRY[0]
    ...
    const portsToTry = basePort === PORTS_TO_TRY[0] ? PORTS_TO_TRY : [basePort]
    let instance: Awaited<ReturnType<typeof serve>> | null = null
    let actualPort: number | null = null

    for (const port of portsToTry) {
      const inst = serve(h3App.app, {
    ...
    if (!instance || actualPort == null) {
      log.error('All ports 6121–6125 are in use. Close other UNICHAN/tamagotchi windows and try again.')
      return
    }
    ...
    resolvedWebSocketUrl = `${protocol}://localhost:${actualPort}/ws`
    if (hostname === '0.0.0.0') {
      log.log(`@proj-airi/server-runtime started on ${protocol}://0.0.0.0:${actualPort}`)
    }
    else {
      log.log(`@proj-airi/server-runtime started on ${protocol}://${hostname}:${actualPort}`)
    }
...
export function setupServerChannelHandlers() {
  ...
  defineInvokeHandler(context, electronStartWebSocketServer, async (req) => {
    ...
    return resolvedWebSocketUrl ?? undefined
  })
  ...
  defineInvokeHandler(context, electronGetWebSocketServerUrl, () => {
    return resolvedWebSocketUrl ?? undefined
  })
}
