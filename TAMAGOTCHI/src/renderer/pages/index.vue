...
import FloatingChatBubbles from '../components/FloatingChatBubbles.vue'
import ControlsIsland from '../components/stage-islands/controls-island/index.vue'
import ResourceStatusIsland from '../components/stage-islands/resource-status-island/index.vue'

import { electronOpenSettings } from '../../shared/eventa'
import { electron as electronNamespace } from '../../shared/electron'
...
const setIgnoreMouseEvents = useElectronEventaInvoke(electronNamespace.window.setIgnoreMouseEvents)
const openSettings = useElectronEventaInvoke(electronOpenSettings)

const { scale, positionInPercentageString } = storeToRefs(useLive2d())
const { live2dLookAtX, live2dLookAtY } = storeToRefs(useWindowStore())
const { fadeOnHoverEnabled } = storeToRefs(useControlsIslandStore())
const { showInlineChat } = storeToRefs(useInlineChatStore())
...
watch(enabled, async (val) => {
  ...
  if (val) {
    try {
      ...
      await startStream()
      if (!stream.value) {
        await until(stream).toBeTruthy({ timeout: 5000 })
      }
    }
    catch (e) {
      console.warn('[Main Page] Mic not available. Grant microphone access (Hearing) and pick a device.', e)
      toast.error('Microphone not available. Click the mic button, grant access if asked, and pick a device.')
      return
    }
...
<template>
  ...
  >
    ...
    >
      ...
      <!-- Menu/controls outside fade so Settings gear is always visible -->
      <ControlsIsland ref="controlsIslandRef" />
      <!-- Floating chat bubbles: pop up over the character, smooth enter -->
      <Transition
      ...
      >
        <FloatingChatBubbles v-if="showInlineChat" class="absolute inset-0 rounded-2xl overflow-visible" />
      </Transition>
    </div>
    <div v-show="isLoading" h-full w-full>
      <div class="absolute left-0 top-0 z-99 h-full w-full flex cursor-grab items-center justify-center overflow-hidden">
        ...
        >
          ...
          <button
            type="button"
            class="cursor-pointer rounded-lg border border-neutral-300 dark:border-neutral-600 bg-white dark:bg-neutral-800 px-3 py-1.5 text-sm font-medium text-neutral-700 dark:text-neutral-200 hover:bg-neutral-100 dark:hover:bg-neutral-700"
            @click="openSettings()"
          >
            Open Settings / Menu
          </button>
