...
export const electronStartWebSocketServer = defineInvokeEventa<string | undefined, { websocketSecureEnabled: boolean }>('eventa:invoke:electron:start-websocket-server')
...
export const electronGetWebSocketServerUrl = defineInvokeEventa<string | undefined>('eventa:invoke:electron:get-websocket-server-url')
...
// Unichan config (read/write ~/.unichan/config.json from Settings)
export interface UnichanConfigPayload {
  providers?: Record<string, { apiKey?: string }>
  agents?: { defaults?: { model?: string, personality?: string, workspace?: string } }
  channels?: { telegram?: { enabled?: boolean, token?: string, allowFrom?: string[] } }
  gateway?: { port?: number, baseUrl?: string, apiKey?: string, sessionKey?: string }
}
export const unichanConfigGet = defineInvokeEventa<UnichanConfigPayload>('eventa:invoke:unichan:config:get')
export const unichanConfigSave = defineInvokeEventa<void, UnichanConfigPayload>('eventa:invoke:unichan:config:save')

/** Test gateway reachability from main process (avoids renderer CORS). */
export const unichanTestGateway = defineInvokeEventa<
  { ok: boolean, status?: number, message: string },
  { baseUrl: string, token: string }
>('eventa:invoke:unichan:test-gateway')
