...
// Show configured providers OR any transcription provider that has credentials (so "success" on provider page actually shows here)
const transcriptionProvidersToShow = computed(() => {
  const all = allAudioTranscriptionProvidersMetadata.value
  return all.filter((m) => {
    if (configuredProviders.value[m.id])
      return true
    if (m.id === 'openai-compatible-audio-transcription') {
      const c = providersStore.getProviderConfig(m.id)
      const hasKey = c && typeof c.apiKey === 'string' && c.apiKey.trim().length > 0
      return !!hasKey
    }
    return false
  })
...
const settingsAudioStore = useSettingsAudioDevice()
const { stopStream, startStream, askPermission } = settingsAudioStore
const { audioInputs, selectedAudioInput, stream } = storeToRefs(settingsAudioStore)
const micPermissionRequesting = ref(false)
const micPermissionError = ref<string>('')
...
async function setupAudioMonitoring() {
  try {
    ...
    // Ensure permission and valid device so startStream() can get the mic (same as main app)
    await askPermission()
    if (audioInputs.value.length > 0 && !selectedAudioInput.value) {
      selectedAudioInput.value = audioInputs.value.find(d => d.deviceId === 'default')?.deviceId || audioInputs.value[0].deviceId
    }
    const current = selectedAudioInput.value
    const isValidDevice = current && audioInputs.value.some(d => d.deviceId === current)
    if (!isValidDevice) {
      error.value = 'No microphone selected or device list empty. Use "Allow microphone & load devices" above, then pick a device.'
      return false
    }

    const mediaStream = await startStream()
    const streamToUse = mediaStream ?? stream.value
    if (!streamToUse) {
      error.value = 'Could not start microphone. Check that the device is connected and you allowed access.'
      return false
    }
    const audioTracks = streamToUse.getAudioTracks()
    if (!audioTracks.length || audioTracks.every(t => t.readyState === 'ended')) {
      error.value = 'Microphone stream has no active tracks. Try another device or re-allow access.'
      return false
    }

    const source = audioContext.value.createMediaStreamSource(streamToUse)
    ...
    vadModelError.value = err instanceof Error ? err.message : String(err)
    return false
  }
}
...
async function toggleMonitoring() {
  if (!isMonitoring.value) {
    const ok = await setupAudioMonitoring()
    isMonitoring.value = !!ok
  }
  else {
    await stopAudioMonitoring()
...
watch(activeTranscriptionProvider, async (provider) => {
  ...
  // If they selected a provider that has credentials but store didn't mark it configured, mark it now
  if (!configuredProviders.value[provider]) {
    const c = providersStore.getProviderConfig(provider)
    const hasKey = c && typeof c.apiKey === 'string' && c.apiKey.trim().length > 0
    if (hasKey)
      providersStore.forceProviderConfigured(provider)
  }
...
async function requestMicAndLoadDevices() {
  micPermissionError.value = ''
  micPermissionRequesting.value = true
  try {
    await askPermission()
    if (audioInputs.value.length > 0 && !selectedAudioInput.value) {
      selectedAudioInput.value = audioInputs.value.find(d => d.deviceId === 'default')?.deviceId || audioInputs.value[0].deviceId
    ...
    micPermissionError.value = e instanceof Error ? e.message : String(e)
    console.error('[Hearing] Mic permission failed:', e)
  }
  finally {
    micPermissionRequesting.value = false
  }
}
...
onMounted(async () => {
  ...
  // Request mic permission so the device list is populated (required for dropdown to show your mic)
  await requestMicAndLoadDevices()

  // If no providers show as configured but user has credentials for OpenAI-compatible transcription, mark it configured
  if (configuredTranscriptionProvidersMetadata.value.length === 0) {
    const transcriptionId = 'openai-compatible-audio-transcription'
    const config = providersStore.getProviderConfig(transcriptionId)
    const hasKey = config && typeof config.apiKey === 'string' && config.apiKey.trim().length > 0
    const hasBaseUrl = config && (typeof config.baseUrl === 'string' && config.baseUrl.trim().length > 0)
    const metadata = providersStore.getProviderMetadata(transcriptionId)
    const defaultBaseUrl = metadata?.defaultOptions?.().baseUrl as string | undefined
    if (hasKey && (hasBaseUrl || defaultBaseUrl)) {
      providersStore.forceProviderConfigured(transcriptionId)
    }
  }
})
  ...
  <div flex="~ col md:row gap-6">
    <div bg="neutral-100 dark:[rgba(0,0,0,0.3)]" rounded-xl p-4 flex="~ col gap-4" class="h-fit w-full md:w-[40%]">
      <div flex="~ col gap-4">
        ...
        <div>
          <div v-if="audioInputs.length === 0" class="mb-4 rounded-lg border border-amber-200 bg-amber-50 p-4 dark:border-amber-800 dark:bg-amber-900/20">
            <p class="mb-2 text-sm font-medium text-amber-800 dark:text-amber-200">
              No microphone listed? The browser needs permission to see your devices.
            </p>
            <p class="mb-3 text-xs text-amber-700 dark:text-amber-300">
              Click below to allow microphone access. Your device list will appear and you can pick the mic you use to talk to your character.
            </p>
            <Button
              :disabled="micPermissionRequesting"
              size="sm"
              @click="requestMicAndLoadDevices"
            >
              <span v-if="micPermissionRequesting" class="mr-2 animate-spin" i-solar:spinner-line-duotone />
              {{ micPermissionRequesting ? 'Requesting…' : 'Allow microphone & load devices' }}
            </Button>
            <p v-if="micPermissionError" class="mt-2 text-xs text-red-600 dark:text-red-400">
              {{ micPermissionError }}
            </p>
          </div>
          <FieldSelect
            ...
            :options="audioInputs.map(input => ({
              label: input.label || input.deviceId || 'Microphone',
              value: input.deviceId,
            }))"
            placeholder="Select an audio input device"
            layout="vertical"
          />
          <p v-if="audioInputs.length > 0" class="mt-1 text-xs text-neutral-500 dark:text-neutral-400">
            Same device is used on the main screen when you turn the mic on to talk to your character.
          </p>
        </div>
        ...
        <div flex="~ col gap-4">
          ...
          -->
            <fieldset
              v-if="transcriptionProvidersToShow.length > 0"
              flex="~ row gap-4"
            ...
            >
              <RadioCardSimple
                v-for="metadata in transcriptionProvidersToShow"
                :id="metadata.id"
                :key="metadata.id"
        ...
        <!-- Transcription language: force output language (stops wrong-language results) -->
        <div v-if="activeTranscriptionProvider" class="border-t border-neutral-200 pt-4 dark:border-neutral-700">
          <FieldSelect
            ...
            description="Force the language of transcribed speech. Use English if you're getting wrong-language output (e.g. Georgian or other)."
            :options="[
        ...
        <div v-else class="flex flex-col gap-4">
          ...
          <div class="space-y-3">
            ...
            <div v-if="activeTranscriptionProvider" class="text-xs text-neutral-500 dark:text-neutral-400">
              <div>Provider: <span class="font-medium">{{ transcriptionProvidersToShow.find(p => p.id === activeTranscriptionProvider)?.localizedName || activeTranscriptionProvider }}</span></div>
