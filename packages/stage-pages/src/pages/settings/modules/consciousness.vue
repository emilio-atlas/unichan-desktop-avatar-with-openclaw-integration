...
const showRemoteBrainLink = computed(() => isStageTamagotchi())
...
/** When true, show provider cards and model selection (Cloud/custom API). When false (Tamagotchi + OpenClaw), show only Unichan link. */
const showCloudProviderChoice = computed(() => !(showRemoteBrainLink.value && useOpenClaw.value))

watch(useOpenClaw, (useOpenClawVal, wasOpenClaw) => {
  if (!showRemoteBrainLink.value) return
  if (useOpenClawVal) {
    activeProvider.value = 'openai-compatible'
    activeModel.value = 'openclaw:main'
    providersStore.markProviderAdded('openai-compatible')
  }
  else if (wasOpenClaw === true) {
    // Switched from OpenClaw to Cloud; clear so user picks a cloud provider
...
<template>
  <div bg="neutral-50 dark:[rgba(0,0,0,0.3)]" rounded-xl p-4 flex="~ col gap-4">
    <!-- Tamagotchi only: choose OpenClaw (Unichan brain) OR cloud/custom API. One or the other. -->
    <div v-if="showRemoteBrainLink" class="flex flex-col gap-3">
      <h2 class="text-lg text-neutral-500 md:text-2xl dark:text-neutral-500">
        {{ t('settings.pages.modules.consciousness.connectionType.title') }}
      </h2>
      <div class="flex flex-wrap gap-4">
        <label class="flex items-center gap-2 cursor-pointer">
          <input
            ...
            type="radio"
            :value="true"
            class="rounded-full border-neutral-400"
          >
          <span class="text-sm font-medium">{{ t('settings.pages.modules.consciousness.connectionType.openclaw') }}</span>
        </label>
        <label class="flex items-center gap-2 cursor-pointer">
          <input
            v-model="useOpenClaw"
            ...
            class="rounded-full border-neutral-400"
          >
          <span class="text-sm font-medium">{{ t('settings.pages.modules.consciousness.connectionType.cloud') }}</span>
        </label>
      </div>
      <p class="text-xs text-neutral-500 dark:text-neutral-400">
        {{ useOpenClaw ? t('settings.pages.modules.consciousness.connectionType.openclawHint') : t('settings.pages.modules.consciousness.connectionType.cloudHint') }}
      </p>
      <p v-if="useOpenClaw" class="text-xs text-primary-600 dark:text-primary-400 font-medium">
        {{ t('settings.pages.modules.consciousness.connectionType.openclawNoProvider') }}
      </p>
    </div>

    <!-- When OpenClaw: only show link to configure connection (no provider list) -->
    <div v-if="showRemoteBrainLink && useOpenClaw" class="rounded-xl border-2 border-primary-200 dark:border-primary-800 bg-primary-50/50 dark:bg-primary-900/20 p-4">
      <RouterLink
        to="/settings/unichan/"
        class="flex items-center gap-3 rounded-xl border-2 border-neutral-200 bg-white p-4 transition-colors dark:border-neutral-700 dark:bg-neutral-900/40 hover:border-primary-400 dark:hover:border-primary-600"
      >
        <div i-solar:server-bold-duotone class="text-2xl text-primary-500 dark:text-primary-400" />
        <div class="flex flex-col text-left">
          <span class="font-medium">{{ t('settings.pages.modules.consciousness.connectionType.configureOpenclaw') }}</span>
          <span class="text-sm text-neutral-400 dark:text-neutral-500">{{ t('settings.pages.modules.consciousness.connectionType.configureOpenclawDescription') }}</span>
        </div>
        <div i-solar:arrow-right-line-duotone class="ml-auto text-xl text-neutral-400 dark:text-neutral-500" />
      </RouterLink>
    </div>

    <!-- When Cloud (or not Tamagotchi): show provider and model selection -->
    <div v-if="showCloudProviderChoice">
      <div flex="~ col gap-4">
        <div>
          <h2 class="text-lg text-neutral-500 md:text-2xl dark:text-neutral-500">
    ...
    <!-- Model selection section (only when using cloud/custom provider, not OpenClaw) -->
    <div v-if="showCloudProviderChoice && activeProvider && supportsModelListing">
      <div flex="~ col gap-4">
        <div>
    ...
    <div v-else-if="showCloudProviderChoice && activeProvider && !supportsModelListing">
      <div flex="~ col gap-4">
        <div>
    ...
    <!-- When Cloud: link to switch to OpenClaw (Tamagotchi only) -->
    <div v-if="showRemoteBrainLink && !useOpenClaw" class="mt-4">
      <RouterLink
        to="/settings/unichan/"
        class="flex items-center gap-3 rounded-xl border-2 border-neutral-200 bg-white p-4 transition-colors dark:border-neutral-700 dark:bg-neutral-900/40 hover:border-primary-400 dark:hover:border-primary-600"
      >
        <div i-solar:server-bold-duotone class="text-2xl text-neutral-500 dark:text-neutral-400" />
        <div class="flex flex-col text-left">
          <span class="font-medium">{{ t('settings.pages.modules.consciousness.sections.section.provider-model-selection.remote_brain_link') }}</span>
          <span class="text-sm text-neutral-400 dark:text-neutral-500">{{ t('settings.pages.modules.consciousness.sections.section.provider-model-selection.remote_brain_link_description') }}</span>
        </div>
        <div i-solar:arrow-right-line-duotone class="ml-auto text-xl text-neutral-400 dark:text-neutral-500" />
      </RouterLink>
    </div>
