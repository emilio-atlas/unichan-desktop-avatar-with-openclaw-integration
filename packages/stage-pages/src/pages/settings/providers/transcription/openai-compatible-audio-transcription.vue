...
onMounted(async () => {
  ...
  // Initialize baseUrl with default if not set (so validation passes with API key only for OpenAI)
  const storedBaseUrl = providers.value[providerId]?.baseUrl
  if (!storedBaseUrl || (typeof storedBaseUrl === 'string' && !storedBaseUrl.trim())) {
    const metadata = providersStore.getProviderMetadata(providerId)
...
// Save and use this provider for Hearing (select it and go to Hearing module)
async function saveAndUseForHearing() {
  if (!isValid.value) {
    forceValid()
  }
  // Always mark as configured in the store so Hearing page shows this provider (UI validation doesn't update store)
  providersStore.forceProviderConfigured(providerId)
  hearingStore.activeTranscriptionProvider = providerId
  const providerConfig = providersStore.getProviderConfig(providerId)
  const modelToUse = (providerConfig?.model as string) || model.value || 'whisper-1'
  if (!modelToUse || !isValidTranscriptionModel(modelToUse)) {
    model.value = 'whisper-1'
  }
  hearingStore.activeTranscriptionModel = (providerConfig?.model as string) || model.value || 'whisper-1'
  hearingStore.activeCustomModelName = hearingStore.activeTranscriptionModel
  await router.push('/settings/modules/hearing')
}
</script>
  ...
  >
    <div flex="~ col md:row gap-6">
      <ProviderSettingsContainer class="w-full md:w-[40%]">
        ...
        <div class="mt-4">
          <Button class="w-full" @click="saveAndUseForHearing">
            {{ isValid ? 'Save and use for Hearing' : 'Use for Hearing anyway' }}
          </Button>
          <p class="mt-2 text-xs text-neutral-500 dark:text-neutral-400">
            {{ isValid ? 'This will select this provider in the Hearing module so your microphone uses it for speech-to-text.' : 'Mark this provider as configured and open Hearing so you can select it (fixes "No Providers Configured" if you already entered your key).' }}
          </p>
        </div>
      </ProviderSettingsContainer>
