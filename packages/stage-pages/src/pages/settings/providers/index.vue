...
const showChatBrainCard = isStageTamagotchi()
...
<template>
  <div mb-6 flex flex-col gap-5>
    <!-- Tamagotchi: one clear place to set the chat brain (OpenClaw) -->
    <RouterLink
      v-if="showChatBrainCard"
      to="/settings/unichan/"
      class="flex items-center gap-4 rounded-xl border-2 border-primary-300 dark:border-primary-700 bg-primary-50/80 dark:bg-primary-950/50 p-4 transition-colors hover:border-primary-500 dark:hover:border-primary-500 hover:bg-primary-100/80 dark:hover:bg-primary-900/40"
    >
      <div class="i-solar:server-bold-duotone text-3xl text-primary-500 dark:text-primary-400 shrink-0" />
      <div class="min-w-0 flex-1">
        <div class="font-semibold text-primary-900 dark:text-primary-100">
          {{ $t('settings.pages.providers.chatBrainCard.title') }}
        </div>
        <div class="text-sm text-primary-700 dark:text-primary-300 mt-0.5">
          {{ $t('settings.pages.providers.chatBrainCard.description') }}
        </div>
      </div>
      <span class="shrink-0 rounded-lg bg-primary-200/80 dark:bg-primary-800/80 px-3 py-1.5 text-sm font-medium text-primary-900 dark:text-primary-100">
        {{ $t('settings.pages.providers.chatBrainCard.action') }}
      </span>
    </RouterLink>
    ...
    <div bg="primary-500/10 dark:primary-800/25" rounded-lg p-4>
      ...
      <div text="primary-700 dark:primary-300" class="flex flex-col gap-2">
        <p v-if="showChatBrainCard">{{ $t('settings.pages.providers.helpinfo.tamagotchiHint') }}</p>
