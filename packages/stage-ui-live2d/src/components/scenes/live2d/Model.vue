...
function setScaleAndPosition() {
  ...
  // Guard against zero/undefined model size (e.g. textures not loaded or model not ready).
  const safeWidth = Math.max(initialModelWidth.value, 1)
  const safeHeight = Math.max(initialModelHeight.value, 1)

  const heightScale = (props.height * 0.95 / safeHeight * offsetFactor)
  const widthScale = (props.width * 0.95 / safeWidth * offsetFactor)
  ...
  // Models with very large internal bounds (e.g. 8192px textures) can get scale ~0 and appear invisible.
  const minVisibleScale = 0.15
  if (scale < minVisibleScale) {
    scale = minVisibleScale
  }
...
async function loadModel() {
  ...
  try {
    ...
    if (msg.includes('Unknown settings JSON')) {
      console.warn(
        '[Live2D] This model could not be loaded. Only Cubism 3/4 (model3.json) is supported. '
        + 'Cubism 2 (model2.json) and other formats are not supported.',
        err,
      )
    }
    else {
