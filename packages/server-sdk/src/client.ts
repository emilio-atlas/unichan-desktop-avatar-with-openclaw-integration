...
export class Client<C = undefined> {
  ...
  private maxRetriesLogged = false
  ...
  private async retryWithExponentialBackoff(fn: () => void | Promise<void>) {
    ...
    while (true) {
      if (maxReconnectAttempts !== -1 && attempts >= maxReconnectAttempts) {
        if (!this.maxRetriesLogged) {
          this.maxRetriesLogged = true
          console.warn(
            `[Channel] Maximum retry attempts (${maxReconnectAttempts}) reached for ws. This is the extension/sync channel (port 6121), not the brain. Chat uses HTTP to the gateway (e.g. port 18789).`,
          )
        }
        return
      }
    ...
    this.connectAttempt = new Promise((resolve, reject) => {
      ...
      ws.onopen = () => {
        settle(() => {
          ...
          // Always authenticate first so the server can mark the peer authenticated (required for context:update etc.).
          // When the server has no auth token, it still responds with authenticated so we can then announce.
