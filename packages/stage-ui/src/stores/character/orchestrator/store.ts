...
export const useCharacterOrchestratorStore = defineStore('character-orchestrator', () => {
  ...
  const sparkNotifyAgent = setupAgentSparkNotifyHandler({
    ...
    onReactionEnd: (eventId, text) => {
      characterStore.onSparkNotifyReactionStreamEnd(eventId, text)
      if (text?.trim()) {
        const sessionId = chatSessionStore.activeSessionId
        if (sessionId) {
          chatSessionStore.ensureSession(sessionId)
          const current = chatSessionStore.getSessionMessages(sessionId)
          const assistantMsg = {
          ...
          chatSessionStore.setSessionMessages(sessionId, [...current, assistantMsg])
        }
      }
    },
