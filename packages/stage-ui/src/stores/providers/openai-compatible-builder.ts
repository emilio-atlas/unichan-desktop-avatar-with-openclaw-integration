...
/** True when base URL is localhost/127.0.0.1 (e.g. Unichan gateway); API key can be omitted. */
...
export function buildOpenAICompatibleProvider(
  ...
  const defaultCapabilities = {
    listModels: async (config: Record<string, unknown>) => {
      ...
      const rawKey = normalizeString(config.apiKey)
      const baseUrl = normalizeBaseUrl(config.baseUrl)
      // For localhost (e.g. Unichan gateway), API key is optional
      const apiKey = rawKey || (isLocalBaseUrl(baseUrl) ? '' : rawKey)
  ...
  const finalValidators = validators || {
    validateProviderConfig: async (config: Record<string, unknown>) => {
      ...
      const isLocal = baseUrl ? isLocalBaseUrl(normalizeBaseUrl(baseUrl)) : false

      // API key required only for non-local URLs (localhost/127.0.0.1 can omit key, e.g. Unichan gateway)
      if (!apiKey && !isLocal) {
