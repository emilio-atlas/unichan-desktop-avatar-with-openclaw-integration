...
export const useHearingStore = defineStore('hearing-store', () => {
  ...
  /** ISO-639-1 language code for transcription (e.g. 'en'). Forces output language and avoids wrong-language results. */
  const transcriptionLanguage = useLocalStorageManualReset<string>('settings/hearing/transcription-language', 'en')
...
export const useHearingSpeechInputPipeline = defineStore('modules:hearing:speech:audio-input-pipeline', () => {
  ...
  const { activeTranscriptionProvider, activeTranscriptionModel, transcriptionLanguage } = storeToRefs(hearingStore)
  const providersStore = useProvidersStore()
  ...
  async function transcribeForMediaStream(stream: MediaStream, options?: {
    ...
    try {
      ...
      const providerConfig = providersStore.getProviderConfig(providerId) || {}
      const language = (options?.providerOptions?.language as string) || (providerConfig.language as string) || transcriptionLanguage.value || 'en'
  ...
  async function transcribeForRecording(recording: Blob | null | undefined) {
    ...
    try {
      if (recording && recording.size > 0) {
        ...
        // Get model and language so transcription uses the right language (avoids wrong-language output)
        const model = activeTranscriptionModel.value
        const providerConfig = providersStore.getProviderConfig(providerId) || {}
        const language = (providerConfig.language as string) || transcriptionLanguage.value || 'en'
        const result = await hearingStore.transcription(
          providerId,
          provider,
          model,
