...
export type BrainConnectionResolver = () => { baseUrl: string, apiKey: string } | null
...
export const useProvidersStore = defineStore('providers', () => {
  ...
  /** When set, getProviderInstance('openai-compatible') uses this for config (consciousness gateway). */
  const brainConnectionResolver = ref<BrainConnectionResolver | null>(null)
  const { t } = useI18n()
    ...
    'openai-audio-speech': buildOpenAICompatibleProvider({
      ...
      validators: {
        validateProviderConfig: (config) => {
          ...
          const errors = [
            !baseUrl && new Error('Base URL is required. Default to https://api.openai.com/v1/ for official OpenAI API.'),
            !isLocal && !config.apiKey && new Error('API Key is required for non-local endpoints. For local (e.g. Unichan gateway) leave empty.'),
          ].filter(Boolean)
          ...
          return {
            errors,
            reason: errors.filter(e => e).map(e => String(e)).join(', ') || '',
            valid: !!baseUrl && (!!config.apiKey || isLocal),
          }
        },
      },
    }),
  ...
  async function validateProvider(providerId: string): Promise<boolean> {
    ...
    // Use default baseUrl for OpenAI-compatible transcription when only API key is set (so validation passes)
    if (providerId === 'openai-compatible-audio-transcription' && config) {
      const defaultOpts = getDefaultProviderConfig(providerId)
      if ((!config.baseUrl || (typeof config.baseUrl === 'string' && !config.baseUrl.trim())) && defaultOpts.baseUrl) {
        config = { ...config, baseUrl: defaultOpts.baseUrl }
      }
    }
  ...
  function forceProviderConfigured(providerId: string) {
    ...
    const state = providerRuntimeState.value[providerId]
    if (state) {
      state.isConfigured = true
      const config = providerCredentials.value[providerId]
      if (config) {
        state.validatedCredentialHash = JSON.stringify(config)
      }
    }
    markProviderAdded(providerId)
  }
  ...
  // Function to get localized provider metadata (returns undefined if providerId empty or unknown)
  ...
  >(providerId: string): Promise<R> {
    if (!providerId || !String(providerId).trim())
      throw new Error('Please select a chat provider in Settings (Consciousness / LLM).')
    // When using the brain resolver, never use cache so we always get the latest token from consciousness
    const useBrainResolver = providerId === 'openai-compatible' && brainConnectionResolver.value
    const cached = useBrainResolver ? undefined : (providerInstanceCache.value[providerId] as R | undefined)
    if (cached)
      return cached
    ...
    // Brain/gateway connection from consciousness (Unichan) overrides providerCredentials for openai-compatible
    let config = providerCredentials.value[providerId]
    if (providerId === 'openai-compatible' && brainConnectionResolver.value) {
      const brain = brainConnectionResolver.value()
      if (brain?.baseUrl)
        config = { baseUrl: brain.baseUrl, apiKey: brain.apiKey ?? '' }
    }
    ...
    if (!config && providerId !== 'browser-web-speech-api') {
      const hint = providerId === 'openai-compatible'
        ? ' Set the brain URL in Settings → Unichan (Another computer) and Save, then try again.'
        : ''
      throw new Error(`Provider credentials for ${providerId} not found.${hint}`)
    }
  ...
  function setBrainConnectionResolver(resolver: BrainConnectionResolver | null) {
    brainConnectionResolver.value = resolver
  }
