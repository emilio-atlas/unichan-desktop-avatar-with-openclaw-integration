...
import { getVRMIdleAnimationUrl } from '@proj-airi/stage-ui-three/assets/vrm'
...
const vrmIdleAnimationUrl = computed(() => getVRMIdleAnimationUrl(vrmIdlePose.value))
...
const speechPipeline = createSpeechPipeline<AudioBuffer>({
  tts: async (request, signal) => {
    ...
    const speechProviderId = activeSpeechProvider.value?.trim()
    if (!speechProviderId)
      return null

    const provider = await providersStore.getProviderInstance(speechProviderId) as SpeechProviderWithExtraOptions<string, UnElevenLabsOptions>
    if (!provider) {
  ...
  <div relative>
    <div h-full w-full>
      ...
      <ThreeScene
        ...
        v-model:state="componentState"
        :model-src="stageModelSelectedUrl"
        :idle-animation="vrmIdleAnimationUrl"
