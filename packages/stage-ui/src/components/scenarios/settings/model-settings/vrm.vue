...
import { VRM_IDLE_POSE_OPTIONS } from '@proj-airi/stage-ui-three/assets/vrm'
...
const { vrmIdlePose } = storeToRefs(vrmSettings)
...
<template>
  ...
  >
    ...
    <div class="mb-4 flex flex-col gap-1 px-2">
      <label class="text-xs text-neutral-500 dark:text-neutral-400">Idle pose</label>
      <select
        v-model="vrmIdlePose"
        class="rounded border border-neutral-300 bg-white px-2 py-1.5 text-sm dark:border-neutral-600 dark:bg-neutral-800"
      >
        <option v-for="opt in VRM_IDLE_POSE_OPTIONS" :key="opt.id" :value="opt.id">
          {{ opt.label }}
        </option>
      </select>
      <p class="text-xs text-neutral-500 dark:text-neutral-400">
        Default pose when the character is idle. Add more .vrma files in stage-ui-three to get more options.
      </p>
    </div>
