...
const canProceed = computed(() => {
  ...
  if (needsApiKey.value && !apiKey.value.trim()) {
    // OpenAI-compatible with localhost base URL (e.g. Unichan gateway) does not require an API key
    if (context.selectedProvider.value?.id === 'openai-compatible' && baseUrl.value.trim()) {
      try {
