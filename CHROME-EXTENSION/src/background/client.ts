...
import type { ExtensionSettings, ExtensionStatus, PageContextPayload, SubtitlePayload, VideoContextPayload, VisionFramePayload } from '../shared/types'
...
export async function ensureClient(
  ...
  options?: { onRequestVisionFrame?: () => void },
) {
  if (!settings.enabled) {
  ...
  client.onEvent('spark:emit', (event) => {
    if ((event.data as { note?: string })?.note === 'request-vision-frame') {
      state.lastVisionFrameAt = Date.now()
      options?.onRequestVisionFrame?.()
    }
  })
...
export function handleVisionFrame(state: ClientState, settings: ExtensionSettings, payload: VisionFramePayload) {
  ...
  sendContextUpdate(state, {
    strategy: ContextUpdateStrategy.ReplaceSelf,
    lane: 'web:vision',
    text: 'Browser screenshot (current tab).',
    metadata: {
      source: 'web-extension',
      site: payload.site,
      url: payload.url,
      ...
      imageMimeType: imageMimeFromDataUrl(payload.dataUrl),
    },
  } as ContextUpdate<Record<string, unknown>>)
}
