...
import { clearError, onBackgroundStatus, reconnect, requestCurrentTabContext, requestStatus, requestVisionFrame, toggleEnabled, updateSettings } from '../../../src/popup/bridge'
...
export const usePopupStore = createGlobalState(() => {
  ...
  function init() {
    ...
    void refresh().then(async () => {
      if (form.enabled && !connected.value)
        await reconnectNow()
      requestCurrentTabContext().catch(() => {})
    })
